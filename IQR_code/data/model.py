...
def generate_exogeneous_variables(n, p, cov_sqrt, dist):
	...
	if dist == 'normal':
		...
		u = np.random.normal(0, 1, (n, p))
		u_flat = u.flatten()
		n_contaminated = int(n * p * 0.05)
		contaminated_idx = np.random.choice(n * p, n_contaminated, replace=False)
		u_flat[contaminated_idx] = np.random.standard_t(1.5, (n_contaminated))
		...
		df = 1.5
		u = np.random.standard_t(df, (n, p))
		u = np.matmul(u, cov_sqrt)
	elif dist == 't_last_laplace':
		u[:, :-1] = np.random.normal(0, 1, (n, p - 1))
		b = 1 / np.sqrt(2)
		u[:, -1] = np.random.laplace(loc=0.0, scale=b, size=n)
		#df = 1
		#u[:, -1] = np.random.standard_t(df, n)

		#u[:, :-1] = np.random.normal(0, 1, (n, p - 1))
		#df = 1.2
		#t_raw = np.random.standard_t(df, n)
		#sigma_left = 0.5
		#sigma_right = 3
		#u[:, -1] = np.where(
		#	t_raw >= 0,
		...
		#u[:, :-1] = np.random.normal(0, 1, (n, p - 1))
		##u[:, :-1] = np.random.uniform(-1, 1, (n, p - 1)) * np.sqrt(3)
		#u[:, -1] = np.random.standard_t(1, n) * 10
		delta = 0.05

		u[:, :-1] = np.random.normal(0, 1, (n, p - 1))
		n_contaminated = int(n * delta)
		contaminated_idx = np.random.choice(n, n_contaminated, replace=False)
		u[:, -1] = np.random.normal(0, 1, n)
		#u[contaminated_idx, -1] = np.random.standard_cauchy(n_contaminated) * 100
		u[contaminated_idx, -1] = np.random.normal(0, 100,n_contaminated)
	else:
...
class StructuralCausalModel5:
	def __init__(self, p, delta_scale=0.1, exogenous_cov=None, exogenous_dist='normal'):
		self.p = p
		if exogenous_cov is None:
			exogenous_cov = np.eye(p)
		exogenous_cov = np.diag(np.diag(exogenous_cov))
	...
	def sample(self, n, split=True):
		u = generate_exogeneous_variables(n, self.p, self.exogenous_cov_sqrt, self.exogenous_dist)
		x = np.copy(u)
		delta = np.random.uniform(0, self.delta_scale, size=3)
		x[:, 3] = u[:, 3]
		x[:, 1] = u[:, 1]
		x[:, 2] = np.sin(x[:, 3]) * 1 + u[:, 2]
		x[:, 0] = x[:, 1] * (3 + delta[0]) + x[:, 2] * (2 + delta[1]) + x[:, 3] * (-2 + delta[2]) + u[:, 0]
		x[:, 4] = np.tanh(x[:, 0]) + u[:, 4]
		if split:
			return x[:, 1:], x[:, :1], x[:, :1] - u[:, :1]
		else:
...
class StructuralCausalModel6:
	...
	def sample(self, n, split=True):
		...
		x[:, 1] = u[:, 1]
		x[:, 2] = np.sin(x[:, 3]) * 1 + u[:, 2]
		x[:, 0] = x[:, 1] * 3 + x[:, 2] * 2 + x[:, 3] * (-0.5) * (u[:, 0] > np.quantile(u[:, 0], 0.9) ) + u[:, 0]
		#x[:, 0] = x[:, 1] * 3 + x[:, 2] * 2 + x[:, 3] * (-0.5) * ((u[:, 0] >= np.quantile(u[:, 0], 0.65)) & (u[:, 0] <= np.quantile(u[:, 0], 0.75))) + u[:, 0]
		#x[:, 5] = x[:, 2] * 4 + np.tanh(x[:, 0]) + u[:, 5]
		if split:
			return x[:, 1:], x[:, :1], x[:, :1] - u[:, :1]
		else:
			return x

class StructuralCausalModel7:
	def __init__(self, p, exogenous_cov=None, exogenous_dist='normal'):
		self.p = p
		if exogenous_cov is None:
			exogenous_cov = np.eye(p)
...
class AdditiveStructuralCausalModel:
	...
	def sample(self, n, split=True):
		...
		elif self.randtype == 't':
			df = 3
			u = np.random.standard_t(df, (n * self.num_vars))/np.sqrt(3)
		elif self.randtype == 'mix':
			u = np.random.normal(0, 1, (n * self.num_vars))
			n_contaminated = int(n * self.num_vars * 0.05)
			contaminated_idx = np.random.choice(n * self.num_vars, n_contaminated, replace=False)
			#u[contaminated_idx] = np.random.normal(0, 10, n_contaminated)
			#u[contaminated_idx] = np.random.standard_t(3, (n_contaminated)) / np.sqrt(3)
			#u[contaminated_idx] = np.random.standard_t(2.1, (n_contaminated)) / np.sqrt(21)
			u[contaminated_idx] = np.random.standard_t(2.5, (n_contaminated))
		else:
			# use unit variance
...
def random_assignment_matrix(num_vars, ratio, function_id_max, coefficient_max, degree_max, reference_g=None):
	...
	function_matrix = np.zeros((num_vars, num_vars), dtype=int)
	coefficient_matrix = np.zeros((num_vars, num_vars), dtype=float)
...
def generate_random_SCM(num_vars, y_index=None, min_child=0, min_parent=0, num_envs=2, nonlinear_id=5, law='linear', same_var=True, randtype='gaussian'):
	if y_index is None:
		...
		for child in child_set:
			while True:
				...
				if np.abs(coeff_mat[child, y_index] - coeff_mat0[child, y_index]) > 0.5 and np.abs(coeff_mat[child, y_index] + coeff_mat0[child, y_index]) > 0.5:
					break
		model = AdditiveStructuralCausalModel(num_vars, coeff_mat, func_mat, y_index, randtype=randtype)
		models.append(model)
